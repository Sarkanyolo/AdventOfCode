...
pub fn d03() {
    ...
    for c in lines[0].chars() {
        ...
        if c == '^' {
            p1_y += 1;
            if is_robot {p2_y2 += 1} else {p2_y1 +=1}
        } else if c == '<' {
            p1_x -= 1;
            if is_robot {p2_x2 -= 1} else {p2_x1 -=1}
        } else if c == '>' {
            p1_x += 1;
            if is_robot {p2_x2 += 1} else {p2_x1 +=1}
        } else if c == 'v' {
        ...
        let tupli2: String;
        if is_robot {
            tupli2 = format!("{}:{}", p2_x2, p2_y2).to_owned();
        } else {
            tupli2 = format!("{}:{}", p2_x1, p2_y1).to_owned();
        }
    ...
    println!("2015 Day 03: {} {}", hm_p1.values().count(), hm_p2.values().count());
}
